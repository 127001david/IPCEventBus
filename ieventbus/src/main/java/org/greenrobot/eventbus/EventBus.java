...
import org.greenrobot.eventbus.service.IPCEventService;
 ...
 * Events are posted ({@link #post(Object)}) to the bus, which delivers it to subscribers that
 * have a matching handler
 * method for the event type.
 * To receive events, subscribers must register themselves to the bus using
 * {@link #register(Object)}.
 * Once registered, subscribers receive events until {@link #unregister(Object)} is called.
 * Event handling methods must be annotated by {@link Subscribe}, must be public, return nothing
 * (void),
...
public class EventBus {
    ...
    /**
     * EventBus原本是不需要初始化的，只需要在适当的时机 register/unregister 就可以接收通知
     * ，但是当跨进程发送通知时需要通过Binder发起IPC，需要一个Service来处理消息分发，所以需要调用初始化方法建立各进程与Service之间的绑定。
     * @param context ctx
     */
    public void init(Context context) {
        context.bindService(new Intent(context, IPCEventService.class), new ServiceConnection() {
            @Override
            public void onServiceConnected(ComponentName name, IBinder service) {
                mIPCEventManager = IPCBusInterface.Stub.asInterface(service);

                try {
                    mIPCEventManager.registerListener(new IIPCEventPostListener.Stub() {
                        @Override
                        public void onReceiveEvent(Bundle event) throws RemoteException {
